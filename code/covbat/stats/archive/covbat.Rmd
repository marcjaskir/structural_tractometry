...
run_covbat_harmonization <- function(
  ...
  covbat_inputs_dir = "/Users/mjaskir/cnt/data/borel/sauce/littlab/users/mjaskir/structural_tractometry/derivatives/covbat/inputs/tract_stats",
  covbat_outputs_dir = "/Users/mjaskir/cnt/data/borel/sauce/littlab/users/mjaskir/structural_tractometry/derivatives/covbat/outputs/tract_stats"
) {
  ...
  if (stat == "volume") {
    colnames(epi_data)[ncol(epi_data)] <- "volume"
  }
  ...
  for (measure in valid_measures) {
    ...
    formula <- y ~ s(age, k = 4, fx = TRUE) + sex
    model <- covfam(
      data = data_measure,
      bat = bat_ctl,
      covar = covar_ctl,
    ...
    if (plot) {
      ...
      before_plot <- ggplot(df_before, aes(x = bat, y = .data[[measure]])) +
        ...
        ggtitle(paste("Before CovBat:", measure)) +
      ...
      after_plot <- ggplot(df_after, aes(x = bat, y = .data[[measure]])) +
        ...
        geom_jitter(width = 0.1) +
        ggtitle(paste("After CovBat:", measure)) +
        ylim(range(c(df_before[[measure]], df_after[[measure]])))

      plot_list[[measure]] <- grid.arrange(before_plot, after_plot, ncol = 2, top = paste(tract, "-", measure))
    }
  }
...
Test case
```{r}
result <- run_covbat_harmonization(
  tract = "C_PO_L",
  stat = "mean",
  plot = FALSE
)
