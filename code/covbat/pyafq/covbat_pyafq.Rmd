...
run_covbat_harmonization <- function(covbat_inputs_dir, tract, train_prop = 0.9) {
  ...
  valid_measures <- names(fromJSON("/mnt/sauce/littlab/users/mjaskir/structural_tractometry/data/metadata/scalar_labels_to_filenames.json"))

  for (measure in valid_measures) {

    # Read input files
    data <- read.csv(file.path(covbat_inputs_dir, tract, paste0(tract, "_", measure, "_data.csv")))
    bat_orig <- read.csv(file.path(covbat_inputs_dir, tract, paste0(tract, "_", measure, "_bat.csv")))
    covar_orig <- read.csv(file.path(covbat_inputs_dir, tract, paste0(tract, "_", measure, "_covar.csv")))
...
covbat_inputs_dir <- paste0("/mnt/sauce/littlab/users/mjaskir/structural_tractometry/derivatives/covbat/inputs/pyafq/", wm_atlas)
